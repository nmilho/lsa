<!DOCTYPE html>
<html lang="pt">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
    <title>Land&Sea Alentejo | Roteiros: Tour e passeio Vintage</title>

    <meta name="description" content="Tour e passeio Vintage">

    <link rel="canonical" href="http://www.landseaalentejo.com/pt/tours/aventura/tourvintage.php">

<!--
    <link rel="alternate" hreflang="en" href="http://www.landseaalentejo.com/en/">
    <link rel="alternate" hreflang="pt" href="http://www.landseaalentejo.com/pt/">
-->

    <meta property="og:locale" content="pt_PT">
    <meta property="og:site_name" content="Land and Sea Alentejo">
    <meta property="og:title" content="Land&Sea Alentejo | Roteiros: Tour e passeio Vintage">
    <meta property="og:url" content="http://www.landseaalentejo.com/pt/tours/aventura/tourvintage.php">
    <meta property="og:type" content="website">
    <meta property="og:description" content="Tour e passeio Vintage">

    <!-- Font Awesome -->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
    <!-- Google Fonts - OpenSans and Oswald -->
    <link href='https://fonts.googleapis.com/css?family=Open+Sans:400,800italic,800,700italic,700,600italic,400italic,600,300italic,300|Oswald:400,300,700' rel='stylesheet' type='text/css'>
    <!-- Bootstrap core CSS -->
    <link href="/css/bootstrap.min.css" rel="stylesheet">
    <!-- Material Design Bootstrap -->
    <link href="/css/mdb.min.css" rel="stylesheet">
    <!-- Your custom carousel slides (optional) -->
    <link href="/css/carousel.css" rel="stylesheet">
    <!-- OWL carousel -->
    <link href="/css/owl.carousel.css" rel="stylesheet">
    <!-- Your custom table styles (optional) -->
    <link href="/css/table_price.css" rel="stylesheet">
    <!-- Your custom styles (optional) -->
    <link href="/css/style.css" rel="stylesheet">

    <!-- GOOGLE ANALYTICS -->
    <!-- Global site tag (gtag.js) - Google Analytics -->
        <script async src="https://www.googletagmanager.com/gtag/js?id=UA-120789359-1"></script>
        <script>
          window.dataLayer = window.dataLayer || [];
          function gtag(){dataLayer.push(arguments);}
          gtag('js', new Date());

          gtag('config', 'UA-120789359-1');
        </script>
    <!-- ./GOOGLE ANALYTICS -->

</head>

<body id="body">

<!-- Start your project here-->

    <!--Main Navigation-->
    <header style="height: auto; margin-top: 75px;" >
        <!--Navbar-->
        <nav class="navbar navbar-expand-lg navbar-dark fixed-top scrolling-navbar" style="background-color: #0f75e6;">
            <div class="container">
            <!-- Navbar brand -->
            <a class="navbar-brand" href="/pt/">
                <img src="/img/logo/logo_h_md.png" height="42" alt="Land&Sea Alentejo">
            </a>

            <!-- Collapse button -->
            <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#topnaVbar" aria-controls="topnaVbar"
                aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button>

            <!-- Collapsible content -->
            <div class="collapse navbar-collapse" id="topnaVbar">

                <!-- Links -->
                <ul class="navbar-nav ml-auto">
                    <li class="nav-item">
                        <a class="nav-link" href="/pt/#body">Início</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link smooth-scroll" href="/pt/about">Quem Somos</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link smooth-scroll" href="/pt/transfers">Transfers</a>
                    </li>

                    <!-- Dropdown -->
                    <li class="nav-item dropdown active">
                        <a class="nav-link dropdown-toggle" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Roteiros</a>
                        <div class="dropdown-menu dropdown-primary" aria-labelledby="navbarDropdownMenuLink">
                            <a class="dropdown-item" href="/pt/tours/aventura">Aventura</a>
                            <a class="dropdown-item" href="/pt/tours/cultura">Culturais</a>
                            <a class="dropdown-item" href="/pt/tours/enoturismo">Enoturismo</a>
                        </div>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link smooth-scroll" href="/pt/contacts">Contatos</a>
                    </li>
                    <!-- Dropdown 
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">PT</a>
                        <div class="dropdown-menu dropdown-primary" aria-labelledby="navbarDropdownMenuLink">
                            <a class="dropdown-item" href="/en/">EN</a>
                            <a class="dropdown-item" href="/pt/">PT</a>
                        </div>
                    </li>-->
                </ul>
                <!-- Links -->
            </div>
            <!-- Collapsible content -->
        </div>
        </nav>
        <!--/.Navbar-->   

    </header>
    <!--Main Navigation-->

    <!--Main layout-->
    <main class="mt-5">

            <!--Section: Tours Index-->
            <section id="toursindex" class="text-center py-5" style="background-color: #fff;">
                <div class="container">
                    <div class="row">
                        <div class="col-md-12 mb-4">
                            <!-- Heading -->
                            <h2 class="title dark-grey-text">Tour Vintage</h2>

                            <div class="card card-image" style="background: url(/photos/tours/aventura/vintage/vintage01.jpg) -0 -350px; height: 300px; background-size: 1110px;">
                                <div class="text-white text-center d-flex align-items-center rgba-black-light py-5 px-4 rounded" style="height: 300px;">
                                    
                                </div>
                              </div>
                        </div>
                    </div>
                    
                    <p class="text-left">Pode passar um dia original e inesquecível a bordo de uma vw clássica.</p>

                    <div class="row">
                        <div class="col-md-12 mb-4">

                            <div class="row">

                                <div class="col-md-7">

                                    <p class="text-left">
                                        O modelo "kombi", que pelo seu aspecto ficou conhecido como "pão de forma", começou a ser vendido em 1950, mas em 1979 deixou de ser produzido no seu país de origem, a Alemanha.
                                    </p>

                                    <p class="text-left">
                                        Vem conhecer o Alentejo numa aventura sem igual!!
                                    </p>

                                    <p class="text-left">
                                        Pode ser alugada para eventos especiais, secções de fotografia, casamentos ou qualquer ocasião.
                                    </p>

                                    <ul class="text-left">
                                        <li>Preços sobe consulta</li>
                                    </ul>

                                </div>

                                <div class="col-md-5">
                                    <figure class="figure">
                                        <img src="/photos/tours/aventura/vintage/vintage02.jpg" class="img-fluid rounded z-depth-1">
                                        <figcaption class="figure-caption">Pão de forma</figcaption>
                                    </figure>
                                </div>

                            </div>
                            
                        </div>

                    </div>

                </div>

            </section>
            <!--/.Section: Tours Index-->





    </main>
    <!--Main layout-->

    
    <!--Footer-->
    <footer class="page-footer font-small blue pt-0">

       
        <!--Footer Links-->
        <div class="container pt-5 mt-5 mb-4 text-center text-md-left">
            <div class="row mt-3">

                <!--First column-->
                <div class="col-md-3 col-lg-4 col-xl-3 mb-4">
                    <h6 class="text-uppercase font-weight-bold">
                        <strong>Land & Sea Alentejo</strong>
                    </h6>
                    <hr class="orange accent-2 mb-4 mt-0 d-inline-block mx-auto" style="width: 60px;">
                    <p>Com o objetivo de dar a conhecer e desenvolver o turismo local - da costa ao mais profundo Alentejo.</p>
                    <!--Facebook-->
                    <p>Siga-nos nas redes sociais:</p>
                    <a href="https://www.facebook.com/Land-Sea-alentejo-545763372449882/" target="_blank" class="btn-floating blue darken-1 fb-ic ml-0">
                        <i class="fa fa-facebook white-text fa-2x mr-4"> </i>
                    </a>
                </div>
                <!--/.First column-->

                <!--Second column-->
                <div class="col-md-2 col-lg-2 col-xl-2 mx-auto mb-4">
                    <h6 class="text-uppercase font-weight-bold">
                        <strong>Transfers</strong>
                    </h6>
                    <hr class="orange accent-2 mb-4 mt-0 d-inline-block mx-auto" style="width: 60px;">
                    <p>
                        <a href="/pt/transfers">Partidas/Chegadas</a>
                    </p>
                    <p>
                        <a href="/pt/transfers">Profissionais</a>
                    </p>
                </div>
                <!--/.Second column-->

                <!--Third column-->
                <div class="col-md-3 col-lg-2 col-xl-2 mx-auto mb-4">
                    <h6 class="text-uppercase font-weight-bold">
                        <strong>Roteiros</strong>
                    </h6>
                    <hr class="orange accent-2 mb-4 mt-0 d-inline-block mx-auto" style="width: 60px;">
                    <p>
                        <a href="/pt/tours/aventura">Aventura</a>
                    </p>
                    <p>
                        <a href="/pt/tours/cultura">Culturais</a>
                    </p>
                    <p>
                        <a href="/pt/tours/enoturismo">Enoturismo</a>
                    </p>
                </div>
                <!--/.Third column-->

                <!--Fourth column-->
                <div class="col-md-4 col-lg-3 col-xl-3">
                    <h6 class="text-uppercase font-weight-bold">
                        <strong>Contatos</strong>
                    </h6>
                    <hr class="orange accent-2 mb-4 mt-0 d-inline-block mx-auto" style="width: 60px;">
                    <p><i class="fa fa-map-marker orange-text"></i> BEJA, 7800, PORTUGAL</p>
                    <p><i class="fa fa-phone orange-text"></i> +351 965 589 291</p>
                    <p><i class="fa fa-envelope orange-text"></i> info@landseaalentejo.com</p>
                    <p><i class="fa fa-id-card orange-text"></i> RNAVT 7728</p>
                </div>
                <!--/.Fourth column-->

            </div>
        </div>
        <!--/.Footer Links-->

        <!--Copyright-->
        <div class="footer-copyright py-3 text-center">
            <div class="container">
                <div class="row">
                    <div class="col-md-10 text-center">
                        <div class="row">
                            <div class="col-md-6 text-center">
                                © 2018 Copyright:
                            <a href="https://www.landseaalentejo.com"> Land&Sea Alentejo</a>
                            </div>
                            <div class="col-md-6 text-center">
                                Powered by: 
                                <a href="https://mdbootstrap.com/bootstrap-tutorial/" target="_blank"> MDBootstrap.com</a>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-2 text-center">
                        <a href="http://www.turismodeportugal.pt/" target="_blank">
                            <img width="70" src="/photos/turismoportugal.png">
                        </a>
                    </div>
                </div>
            </div>
            
            
        </div>
        <!--/.Copyright-->

    </footer>
    <!--/.Footer-->
                                            

    <!--BackToTop-->
    <div class="fixed-action-btn backtotop d-none" style="bottom: 45px; right: 24px;">
        <a href="#body" class="btn-floating btn-lg orange text-center waves-effect waves-light smooth-scroll">
            <i class="fa fa-chevron-up white-text"></i>
        </a>
    </div>
    <!--./BackToTop-->


<!-- /Start your project here-->

    <!-- SCRIPTS -->
    <!-- JQuery -->
    <script type="text/javascript" src="/js/jquery-3.2.1.min.js"></script>
    <!-- Bootstrap tooltips -->
    <script type="text/javascript" src="/js/popper.min.js"></script>
    <!-- Bootstrap core JavaScript -->
    <script type="text/javascript" src="/js/bootstrap.min.js"></script>
    <!-- OWL Carousel JavaScript -->
    <script type="text/javascript" src="/js/owl.carousel.min.js"></script>
    <!-- MDB core JavaScript -->
    <script type="text/javascript" src="/js/mdb.min.js"></script>
    <!-- Smooth-Scroll JavaScript
    <script type="text/javascript" src="/js/scripts.js"></script>
    <!-- Precos Script 
    <script type="text/javascript" src="/js/precos.js"></script>
    <!-- Ajax-form Script 
    <script type="text/javascript" src="/js/app.js"></script>
    -->
</body>

</html>
